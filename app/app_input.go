...
func (m *home) handleMouse(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	...
	if msg.Button == tea.MouseButtonWheelUp || msg.Button == tea.MouseButtonWheelDown {
		// Route to command palette when it is open.
		...
		// Default: scroll the content pane.
...
func (m *home) handleCommandPaletteKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	if m.commandPalette == nil {
		m.state = stateDefault
		return m, nil
	}
	action, closed := m.commandPalette.HandleKeyPress(msg)
	if closed {
		m.commandPalette = nil
		m.state = stateDefault
		if action != "" {
			return m.executeCommandPaletteAction(action)
		}
		return m, nil
	}
	return m, nil
...
func (m *home) handleSettingsKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	if m.settingsOverlay == nil {
		m.state = stateDefault
		return m, nil
	}
	changedKey, closed := m.settingsOverlay.HandleKeyPress(msg)
	if closed {
	...
	if changedKey != "" {
		m.applySettingChange(changedKey)
	}
	return m, nil
}
...
func (m *home) handleDefaultKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case keys.KeyCommandPalette:
		m.commandPalette = overlay.NewCommandPalette(m.buildCommandItems())
