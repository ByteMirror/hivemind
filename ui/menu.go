	...
	case StateEmpty:
		m.rows = []menuRow{
			...
			// Row 2: system
			{
				...
				menuGroup{keys: []keys.KeyName{keys.KeyCommandPalette, keys.KeyHelp, keys.KeyQuit}},
			},
		}
	case StateDefault:
		if m.instance != nil {
...
func (m *Menu) buildInstanceRows() {
	...
	sysGroup := menuGroup{keys: []keys.KeyName{keys.KeyCommandPalette, keys.KeyKillAllInTopic, keys.KeyHelp, keys.KeyQuit}}
